...
public:
	...
	// アクセッサ
	void SetTrans(const DirectX::SimpleMath::Vector3& trans) { m_Obj.SetPos(trans); }
	void SetRot(const DirectX::SimpleMath::Vector3& rot) { m_Obj.SetAngle(rot); }
	void SetScale(float scale) { m_Obj.SetScaling(DirectX::SimpleMath::Vector3(scale)); }
	void SetLocalWorld(const DirectX::SimpleMath::Matrix& mat) { m_Obj.SetWorld(mat); }

	const DirectX::SimpleMath::Vector3& GetTrans() { return m_Obj.GetPos(); }
	const DirectX::SimpleMath::Vector3& GetRot() { return m_Obj.GetAngle(); }
